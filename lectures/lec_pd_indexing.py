...
df.sort_index(inplace=True)
x = df.loc[['b','d', 'f', 'j'], 'col1':'col1'] # returns a dataframe
# or x = df.loc[['b','d', 'f', 'j'], ['col1']]
print(x)
...
# df.iloc[row position] --> series if abs(pos) < len(df.index)
...
x  = df.iloc[:, 0] # 0 as column index = 1st column
# x  =df.iloc[:-1,1] last column
print(x)
...
# This will return a *dataframe* with the first row of df & all col
x = df.iloc[0:1,:]
print(x)
...
x  = df.iloc[2:, :] # start from 3rd row
print(x)

# Set x to be a series with all columns of the first row
x  = df.iloc[0, 0:] # select 1st row, start from beginning
print(x)
...
rows = list(range(0, len(df), 2)) # Get the even row index
x = df.iloc[rows, [1]]
print(x)
...
x = df['2020-01-02':'2020-01-03']   # row labels: operates on row index, not col
print(x)
